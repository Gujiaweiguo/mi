...
{
    public class ConfirmUnionMod : IConfirmVoucher
    {
        /**
        * 确认单据后对数据生效的处理
        */
        public void ValidVoucher(int voucherID, int operType, BaseTrans trans)
        {
            if (operType == FuncApp.OPER_TYPE_CONFIRM)
            {
                ...
                Union.ConUnion conUnion = new Lease.Union.ConUnion();
                ...
                /*相关条款*/
                baseBO.WhereClause = "ConModID = " + voucherID;
                rsConLease = baseBO.Query(conUnionMod);
                if (rsConLease.Count == 1)
                {
                    conUnionMod = rsConLease.Dequeue() as ConUnionMod;

                    conUnion.BillCycle =conUnionMod.BillCycle;
                    conUnion.CurTypeID =conUnionMod.CurTypeID;
                    conUnion.AccountCycle =conUnionMod.AccountCycle;
                    conUnion.RentInc =conUnionMod.RentInc;
                    conUnion.TaxRate =conUnionMod.TaxRate;
                    conUnion.TaxType =conUnionMod.TaxType;
                    conUnion.InTaxRate =conUnionMod.InTaxRate;
                    conUnion.OutTaxRate =conUnionMod.OutTaxRate;
                    
                    trans.WhereClause = "ContractID = " + conUnionMod.ContractID;
                    if (trans.Update(conUnion) == -1)
                    {
                        trans.Rollback();
                        return;
                    }
