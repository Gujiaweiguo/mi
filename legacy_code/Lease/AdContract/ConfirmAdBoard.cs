...
{
    public class ConfirmAdBoard : IConfirmVoucher
    {
        /**
        * 确认单据后对数据生效的处理
        */
        public void ValidVoucher(int voucherID, int operType, BaseTrans baseTrans)
        {
            if (operType == FuncApp.OPER_TYPE_CONFIRM)
            {
                ...
                baseTrans.ExecuteUpdate("Update ConAdBoard Set ConAdBoardStatus = " + ConAdBoard.BLANKOUT_STATUS_LEASEOUT + " Where ContractID =" + voucherID);
            }
        }
