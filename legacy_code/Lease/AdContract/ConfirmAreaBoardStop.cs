...
{
    public class ConfirmAreaBoardStop : IConfirmVoucher
    {
        /**
        * 确认单据后对数据生效的处理
        */
            ...
            if (operType == FuncApp.OPER_TYPE_CONFIRM)
            {
                ...
                if (rs.Count == 1)
                {
                    ...
                    if (updateContractStopDate.ConEndDate >= updateContractStopDate.TerDate)
                    {
                        ...
                        if (baseTrans.ExecuteUpdate("Update ConArea Set ConAreaStatus = " + ConArea.BLANKOUT_STATUS_INVALID + " Where ContractID =" + updateContractStopDate.ContractID) == -1)
                        {
                            baseTrans.Rollback();
                            return;
                        }
