...
function NlsTree(tId) {
  ...
  for (var i=0;i<asc.length;i++) {
    if (asc[i].src.toLowerCase().indexOf("nlstree.js")>=0) {this.defImgPath=asc[i].src.replace(/nlstree.js/gi, "img/");break;}
  }
...
NLSTREE.expandNode = function(id) {
  ...
  if ((!sNd.exp && sNd.fc) || (sNd.svrLoad && sNd.loaded==0)) this.toggleNode(sNd.id);
};
