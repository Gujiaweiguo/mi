...
function calendarExt(extProc) //主调函数,需要最后处理
{
    WebCalendar.extProc=extProc;
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        returnValue = flag ? a[2] +d[2]+ appendZero(a[1]) +d[2]+ appendZero(1):a[2] +d[2]+ a[1] +d[2]+ a[0];
        ...
        if(WebCalendar.extProc!=null){
            WebCalendar.extProc();
            WebCalendar.extProc = null;
        }
    }
}
