...
public partial class ReportM_RptInv_RptInvChargeHeader : BasePage
{
    ...
    private void BindData()
    {
        ...
        discreteValue[0].Value = (String)GetGlobalResourceObject("BaseInfo", "Menu_ContractExpensesSum");//标题
        ...
        discreteValue[12].Value = (String)GetGlobalResourceObject("BaseInfo", "PotShop_lblPotShopName");//商铺名称
        ...
        {
            strWhere = strWhere + " AND Contract.SubsID='" + this.ddlSubs.SelectedValue + "'";
        }
        if (ddlBizproject.Text != "")//商业项目
        {
            strWhere = strWhere + " AND Conshop.StoreID='" + ddlBizproject.SelectedValue + "'";
        }
        if (this.ddlBuildingName.Text != "")//大楼名称
        {
            strWhere = strWhere + " AND Conshop.BuildingID='" + this.ddlBuildingName.SelectedValue + "'";
        }
        if (this.ddlFloorName.Text != "")//楼层名称
        {
            strWhere = strWhere + " AND Conshop.FloorID='" + this.ddlFloorName.SelectedValue + "'";
        }
        if (this.txtConractCode.Text.Trim() != "")//合同号
        {
            strWhere = strWhere + " AND Contract.ContractCode='" + this.txtConractCode.Text.Trim() + "'";
        }
        if (this.ddlContractStatus.Text != "")//合同状态
        {
            strWhere = strWhere + " AND Contract.ContractStatus='" + this.ddlContractStatus.SelectedValue + "'";
        }
        ...
        strWhere = strWhere + strAuth;
        string str_sql = @"SELECT Contract.ContractCode,Customer.CustShortName as CustName,Contract.ContractID,Contract.SubsID,Subsidiary.SubsName,Conshop.StoreID,Store.StoreName,Conshop.BuildingID,Building.BuildingName,Conshop.FloorID,Floors.FloorName,Conshop.ShopID,
ConShop.ShopCode,ConShop.ShopName,Contract.ConStartDate,Contract.ConEndDate,Contract.ContractStatus,sPay.ChargeTypeName,ISNULL(sPay.InvActPayAmt,0) as sAmt,iSNULL(sPay.InvPaidAmt,0) as sactpayAmt,ISNULL(sPay.InvNotPayAmt,0) as sactnotpayAmt FROM invoiceHeader INNER JOIN ( SELECT SUM(InvoiceDetail.InvActPayAmt) AS InvActPayAmt,SUM(InvoiceDetail.InvPaidAmt) as InvPaidAmt,SUM(InvoiceDetail.InvActPayAmt)-SUM(InvoiceDetail.InvPaidAmt) as InvNotPayAmt,ContractID,InvoiceDetail.ChargeTypeID,ct.ChargeTypeName FROM invoiceHeader as invhe INNER JOIN InvoiceDetail ON (InvoiceDetail.InvID = invhe.InvID) INNER JOIN ChargeType as ct  ON (InvoiceDetail.ChargeTypeID = ct.ChargeTypeID) group by ContractID,InvoiceDetail.ChargeTypeID,ct.ChargeTypeName ) as sPay ON (invoiceHeader.contractID = sPay.contractID) INNER JOIN Contract ON (invoiceHeader.contractID = Contract.ContractID ) INNER JOIN Customer ON (Contract.CustID = Customer.CustID) INNER JOIN CurrencyType ON (CurrencyType.CurTypeId=invoiceheader.InvCurTypeId) INNER JOIN  ConShop ON (Contract.contractID = ConShop.contractID) INNER JOIN Store ON (ConShop.StoreID=Store.StoreID) INNER JOIN Subsidiary ON (Contract.SubsID=Subsidiary.SubsID) INNER JOIN Building ON (Conshop.BuildingID=Building.BuildingID) INNER JOIN Floors ON (Conshop.FloorID=Floors.FloorID) where 1=1 " +strWhere+ @" group by Contract.ContractCode,
Customer.CustShortName,Contract.ContractID,Contract.SubsID,Subsidiary.SubsName,Conshop.StoreID,Store.StoreName,Conshop.BuildingID,Building.BuildingName,Conshop.FloorID,Floors.FloorName,Conshop.ShopID,ConShop.ShopCode,ConShop.ShopName,  
Contract.ConStartDate,Contract.ConEndDate,Contract.ContractStatus,sPay.ChargeTypeName,sPay.InvActPayAmt,sPay.InvPaidAmt,sPay.InvNotPayAmt ";
        //str_sql = str_sql + " order by storeid asc";
        Session["paraFil"] = paraFields;
        Session["sql"] = str_sql;
        Session["rpUrl"] = "..\\ReportM\\Report\\Mi\\Inv\\RptInvChargeHeader.rpt";
    }
    /// <summary>
    /// 查询
    /// </summary>
    ...
    {
        this.Response.Redirect("~/ReportM/RptInv/RptInvChargeHeader.aspx");
    }
}
