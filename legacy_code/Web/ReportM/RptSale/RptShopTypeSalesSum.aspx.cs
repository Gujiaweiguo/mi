...
public partial class RptBaseMenu_RptShopTypeSalesSum : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!this.IsPostBack)
        {
            InitDDL();
            BindBizProject();
            baseInfo = (String)GetGlobalResourceObject("BaseInfo", "Menu_ShopTypeSaleStatistics");
    ...
    private void BindData()
    {
        ...
        if (RB1.Checked)
        {
            discreteValue[5].Value = (String)GetGlobalResourceObject("ReportInfo", "RptShopTypeSaleSumday");//标题
        }
        else
        {
            discreteValue[5].Value = (String)GetGlobalResourceObject("ReportInfo", "RptShopTypeSaleSummth");//标题
        }
        ...
        paraField[9].Name = "REXShopTypeName";
        ...
        paraField[10].Name = "REXDate";
        discreteValue[10] = new ParameterDiscreteValue();
        paraField[10].CurrentValues.Add(discreteValue[10]);

        if (this.RB1.Checked)
        {
            discreteValue[10].Value = (String)GetGlobalResourceObject("BaseInfo", "Rpt_Date");//日期
        }
        else
        {
            discreteValue[10].Value = (String)GetGlobalResourceObject("BaseInfo", "Rpt_SalesMonth");//记账月
        }
        ...
        if (this.RB1.Checked)
        {
            str_sql = str_sql + " group by StoreID,StoreName,ShopTypeID,ShopTypeName,BizDate";
        }
        else
        {
            str_sql = str_sql + " group by StoreID,StoreName,ShopTypeID,ShopTypeName,Month ";
        }
        if (RB1.Checked)
        {
            str_sql = str_sql + " order by storeid,ShopTypeID,BizDate ";
        }
        else
        {
            str_sql = str_sql + " order by storeid,ShopTypeID,Month ";
        }
        Session["paraFil"] = paraFields;
        Session["sql"] = str_sql;
        Session["rpUrl"] = "..\\ReportM\\Report\\Mi\\Sale\\RptShopTypeSalesSum.rpt";
    }
   
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        this.Response.Redirect("~/ReportM/RptSale/RptShopTypeSalesSum.aspx");
    }
