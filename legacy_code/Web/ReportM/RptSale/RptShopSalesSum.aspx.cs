...
public partial class RptBaseMenu_RptShopSalesSum : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!this.IsPostBack)
        {
            InitDDL();
            BindBizProject();
            baseInfo = (String)GetGlobalResourceObject("BaseInfo", "Menu_ShopSellStatistics");
    ...
    private void InitDDL()
    {
        ...
        //绑定商铺类型
        baseBo.WhereClause = "ShopTypeStatus = " + ShopType.SHOP_TYPE_STATUS_VALID;
        Resultset rs3 = baseBo.Query(new ShopType());
    ...
    private void BindData()
    {
        ...
        if (RB1.Checked)
        {
            discreteValue[5].Value = (String)GetGlobalResourceObject("ReportInfo", "RptShopSellSumday_Title");//标题
        }
        else
        {
            discreteValue[5].Value = (String)GetGlobalResourceObject("ReportInfo", "RptShopSellSummth_Title");//标题
        }
        ...
        discreteValue[9].Value = (String)GetGlobalResourceObject("BaseInfo", "PotShop_lblPotShopName");//商铺名称
        ...
        if (this.RB1.Checked == true)
        {
            str_sql = "Select StoreID,StoreName,FloorID,FloorName,ShopID,ShopName,convert(varchar(10),BizDate,120) as Date,PaidAmt,TotalReceipt,TotalQty,Convert(Decimal(10,2),(PaidAmt/TotalReceipt)) As ReceiptAve,Convert(Decimal(10,2),(PaidAmt/TotalQty)) As QtyAve From TransShopday Where 1=1 ";
        }
        else
        {
            str_sql = "Select StoreID,StoreName,FloorID,FloorName,ShopID,ShopName,convert(varchar,year(Month))+'-'+convert(varchar,month(Month)) as Date,PaidAmt,TotalReceipt,TotalQty,Convert(Decimal(10,2),(PaidAmt/TotalReceipt)) As ReceiptAve,Convert(Decimal(10,2),(PaidAmt/TotalQty)) As QtyAve From TransShopMth Where 1=1 ";
        }
        ...
        if (ddlShopCode.Text != "")
        {
            str_sql = str_sql + " AND ShopID ='" + ddlShopCode.SelectedValue.ToString() + "'";
        }
        
        if (ddlBuildingName.Text != "")
        {
            str_sql = str_sql + " AND BuildingID = '" + ddlBuildingName.SelectedValue + "'";
        }
        if (ddlFloorName.Text != "")
        {
            str_sql = str_sql + " AND FloorId ='" + ddlFloorName.SelectedValue + "'";
        }
        
        if (this.ddlShopType.Text != "")
        {
            str_sql = str_sql + " AND ShopTypeId ='" + this.ddlShopType.SelectedValue + "'";
        }
        if (ddlTradeID.Text != "")
        {
        ...
        {
            if (this.txtStartBizTime.Text != "" && this.txtEndBizTime.Text.Trim()!="")
            {
                str_sql = str_sql + " AND month >='" + Convert.ToDateTime(this.txtStartBizTime.Text).ToString("yyyy-MM-01") + "' AND month <='" + Convert.ToDateTime(this.txtEndBizTime.Text).ToString("yyyy-MM-01") + "' ";
            }
        }
        ...
        if (RB1.Checked)
        {
            str_sql = str_sql + " order by StoreID,FloorID,ShopID, BizDate";
        }
        else
        {
            str_sql = str_sql + " order by StoreID,FloorID,ShopID,Month";
        }
        Session["paraFil"] = paraFields;
        Session["sql"] = str_sql;
        Session["rpUrl"] = "..\\ReportM\\Report\\Mi\\Sale\\RptShopSalesSum.rpt";
    }
    ...
    protected void RB2_CheckedChanged(object sender, EventArgs e)
    {
        ...
        this.txtStartBizTime.Text = DateTime.Now.Date.ToString("yyyy-MM-dd");
    }
