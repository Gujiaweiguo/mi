...
using Base.Page;
using BaseInfo.Dept;
public partial class ReportM_RptInv_RptStoreBudgetIncomeDetail : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            InitDDL();
            pageTitle = (String)GetGlobalResourceObject("BaseInfo", "Menu_StoreBudgetIncomeDetail");
    ...
    private void BindData()
    {
        ...
        discreteValue[0].Value = (String)GetGlobalResourceObject("BaseInfo", "Menu_StoreBudgetIncomeDetail");
        ...
        paraField[5].Name = "REXYearBudget";//截至该月年度预算情况
        ...
        paraField[8].Name = "RexStoreName";//项目名称
        discreteValue[8] = new ParameterDiscreteValue();
        discreteValue[8].Value = (String)GetGlobalResourceObject("BaseInfo", "Store_StoreName");
        ...
        if (DDLArea.Text != "")
        {
            wheresql = " AND b.deptid ='" + DDLArea.SelectedValue + "'";
        }
        if (DDlCity.Text!="")
        {
            wheresql = " AND a.deptid ='" + DDlCity.SelectedValue + "'";
        }
        ...
        str_sql = "select conshop.storeid, dept.deptname,invoicedetail.period, sum(invoicedetail.invPayAmtL) invPayAmt," +
                    "aa.yearbudgetamt/month(invoicedetail.period) budgetamt,aa.yearbudgetamt,bb.invPayAmt yearInvPayAmt " +
                    "from invoicedetail " +
                    ...
                    "inner join (select storeid,sum(monthamt) yearbudgetamt from budget where startdate <='" + ddlYear.Text.ToString().Trim() + "' and enddate >='" + ddlYear.Text.ToString().Trim() + "' group by storeid) aa " +
                        "on (aa.storeid=conshop.storeid) " +
                    "inner join (select conshop.storeid,sum(invoicedetail.invPayAmtL) invPayAmt from invoicedetail " +
                        "inner join invoiceheader on (invoicedetail.invid=invoiceheader.invid) " +
                        "inner join conshop on (invoiceheader.contractid=conshop.contractid) " +
                        "where invoicedetail.chargetypeid in (select chargetypeid from chargetype where chargeclass in (1,3)) " +
                        "and year(invoicedetail.period)='" + ddlYear.Text.ToString().Trim() + "' " +
                        "group by conshop.storeid) bb on (bb.storeid=conshop.storeid) " +
                    ...
                    "where invoicedetail.chargetypeid in (select chargetypeid from chargetype  " +
                    "where chargeclass in (1,3))  " + wheresql +
                    "group by aa.storeid,aa.yearbudgetamt,conshop.storeid,invoicedetail.period,dept.deptname,bb.invPayAmt " +
                    "order by dept.deptname,invoicedetail.period ";


        Session["paraFil"] = paraFields;
        Session["sql"] = str_sql;
        Session["rpUrl"] = "..\\ReportM\\Report\\Mi\\Group\\RptStoreBudgetIncomeDetail.rpt";
    ...
    protected void DDLArea_SelectedIndexChanged(object sender, EventArgs e)
    {
        BaseBO baseBo = new BaseBO();
        DataSet ds = new DataSet();
        baseBo.WhereClause = "DeptType=" + Dept.DEPT_TYPE_CITY + " and PDeptID=" + DDLArea.SelectedValue;
        baseBo.OrderBy = "orderid";
        ds = baseBo.QueryDataSet(new Dept());
        DDlCity.Items.Clear();
