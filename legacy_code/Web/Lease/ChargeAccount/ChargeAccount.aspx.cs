...
public partial class Lease_ChargeAccount_ChargeAccount : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        baseInfo = (String)GetGlobalResourceObject("BaseInfo", "Lease_lblLeaseAccount");
    ...
    protected void btnQuery_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterClientScriptBlock(Page, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "ChargeAccount_ContractCodeNull") + "'", true);
        }
        else
        {
            ...
            if (rs.Count == 1)
            {
                ...
                if (cmbContractType.SelectedValue == Contract.BIZ_MODE_UNIT.ToString())
                {
                    btnOK.Enabled = false;
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            //查询条件：合同状态-正常或终止（并且终止状态的合同：终止日期>算费日期）；经营方式:租赁
            basebo.WhereClause = " BizMode = " + Contract.BIZ_MODE_LEASE + " and (ContractStatus = " + Contract.CONTRACTSTATUS_TYPE_INGEAR + " or ContractStatus = " + Contract.CONTRACTSTATUS_TYPE_END + " and StopDate > '" + Convert.ToDateTime(txtChargeDate.Text) + "')";
            if (RBtnOneContract.Checked == true)
            {
                basebo.WhereClause = "Contract.ContractID = " + Convert.ToInt32(ViewState["contractid"]) + " and " + basebo.WhereClause;
            }
            ////////add by tjm//////////////////
            string str_sql ="";
            ...
            ////                   " FROM Contract INNER JOIN ConShop ON (Contract.ContractID=ConShop.ContractID) Where 1=1" ;
            ////string  WhereClause = " AND BizMode = " + Contract.BIZ_MODE_LEASE + " and (ContractStatus = " + Contract.CONTRACTSTATUS_TYPE_INGEAR + " or ContractStatus = " + Contract.CONTRACTSTATUS_TYPE_END + " and StopDate > '" + Convert.ToDateTime(txtChargeDate.Text) + "')";
            ////if (RBtnOneContract.Checked == true)
            ////{
            ////    WhereClause = WhereClause + " And ContractID = " + Convert.ToInt32(ViewState["contractid"]);
            ////}
            ...
            //ds = basebo.QueryDataSet(QuerySQL);
            //Resultset rs = basebo.QueryDataSet(QuerySQL);
            ...
            if (rs.Count > 0)
            {
                ...
                foreach (ConLeaseCharge conLeaseCharge in rs)
                {
                    ...
                    int result = ChargeAccount.AccountCharge(conLeaseCharge.ContractID, Convert.ToDateTime(txtChargeDate.Text), 0, chargeTypeValue, htb, bancthID, out invCode, out bfoChgNoAryList);
                        ...
                        if (invCode > 0)
                        {
                            ...
                            if (RBtnPrint.Checked == true)
                            {
                                ...
                                if (invCode > 0)
                                {
                                    ...
                                    //if (i == 1)
                                    //{
                                        this.Response.Redirect("../../ReportM/RptLeaseInv.aspx?InvCode=" + invCode + "&flag =" + 0);
                                    //}
                                }
                                else
                                {
    ...
    private void InsertLog(int conID,int flag,string note,int befFlag,string bancthID)
    {
        ...
        baseBo.WhereClause = "";
        baseBo.WhereClause = "ContractID = " + conID;
        Resultset leaseRS = baseBo.Query(new ConLease());
        ConLease conlse = leaseRS.Dequeue() as ConLease;
