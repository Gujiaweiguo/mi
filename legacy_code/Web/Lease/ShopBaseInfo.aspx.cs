...
public partial class Lease_ShopBaseInfo : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!this.IsPostBack)
        {
            ...
            ds = baseBO.QueryDataSet("Select AutoShopCode From SMSPara");
            if (Convert.ToInt32(ds.Tables[0].Rows[0]["AutoShopCode"]) == SMSPara.AUTO_YES)
            {
            ...
            if (rs.Count == 1)
            {
                ...
                {
                    //this.BtUBlankOut.Visible = false;
                    //this.BtUAdd.Visible = false;
    ...
    {
        if (this.txtRentArea.Text.Trim()=="")
        {
            ScriptManager.RegisterClientScriptBlock(UpdatePanel1, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "ConShopBaseInfo_RentAreaErr") + "'", true);
            GetAllShopInfo(0);
            return;
        }
        if (this.txtStartDate.Text.Trim() == "")
        {
            ScriptManager.RegisterClientScriptBlock(UpdatePanel1, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "Prompt_LeaseContractBeginDate") + "'", true);
            GetAllShopInfo(0);
            return;
        }
        if ( this.txtEndDate.Text.Trim() == "")
        {
            ScriptManager.RegisterClientScriptBlock(UpdatePanel1, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "Prompt_LeaseContractEndDate") + "'", true);
            GetAllShopInfo(0);
            return;
        }
        try
        {
            ...
            if (InsertOrUpdateShopBaseInfo() == -1)    //新增或更新Conshop
            {
            ...
            if (SaveShopUnits() == -1)     //更新ConShopUnit
            {
                ScriptManager.RegisterClientScriptBlock(UpdatePanel1, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "Prompt_WorkLose") + "'", true);
            }
            ScriptManager.RegisterClientScriptBlock(UpdatePanel1, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "Prompt_Success") + "'", true);
    ...
    //        {
                ...
                //    {
                //        string shopCode = ViewState["ContractCode"].ToString() + "_" + DDownListBuilding.Text.Substring(0, DDownListBuilding.SelectedItem.Text.IndexOf(":")) + "_" + DDownListUnit.SelectedItem.Text;
                //        if(shopCode.Length > 16)
                //        {
                //            txtShopCode.Text = shopCode.Substring(0,16);
                //        }
                //        else
                //        {
    ...
    private void BindBusinessItem()
    {
        ...
        BaseBO objBaseBo = new BaseBO();
        BaseInfo.User.SessionUser sessionUser = (BaseInfo.User.SessionUser)Session["UserAccessInfo"];
    ...
    //{
    //    string selected = (String)GetGlobalResourceObject("BaseInfo", "Select_Select");
    //    baseBo.WhereClause = "StoreID=" + Storeid;//根据商业项目绑定大楼
    //    DDownListBuilding.Items.Clear();
    //    rs = baseBo.Query(new Building());
    //    DDownListBuilding.Items.Add(new ListItem(selected, "-1"));
    //    foreach (Building building in rs)
    //        DDownListBuilding.Items.Add(new ListItem(building.BuildingCode + ":" + building.BuildingName, building.BuildingID.ToString()));
    ...
    private void GetAllShopInfo(int FirstStatus)
    {
        ...
        baseBo.WhereClause = "a.ShopTypeID=b.ShopTypeID and ContractID = " + contractID;
    ...
    protected void GetShopBaseInfo(int shopId)
    {
        ...
        if (rs.Count == 1)
        {
            ...
            try { DDownListShopType.SelectedValue = shop.ShopTypeID.ToString(); }
            catch { }
            ...
            txtShopCode.Text = (shop.ShopCode == "0" ? "" : shop.ShopCode.ToString());
            ...
            try { this.ddlStore.SelectedValue = shop.StoreID.ToString(); }
            catch {}
            ...
            {
                if (shop.ShopStatus == ConShop.CONSHOP_TYPE_INGEAR)
                {
                    rdoYes.Checked = true;
                }
                else if (shop.ShopStatus == ConShop.CONSHOP_TYPE_END)
                {
                    rdoNo.Checked = true;
                }
            }
            ...
            //    try { DDownListBuilding.SelectedValue = (shop.BuildingID == null ? "" : shop.BuildingID.ToString()); }
            //    catch { }
            //}
            //else
            //    BindBuilding(-1);
    ...
    protected void FillShopBaseInfo()
    {
        ...
        {
            conShop = new ConShop();
            conShop.ShopCode = txtShopCode.Text.Trim();
            conShop.ShopName = txtShopName.Text.Trim();
            ...
            try { conShop.RentArea = Convert.ToDecimal(txtRentArea.Text); }
            catch { }
            conShop.AreaId = Convert.ToInt32(this.DDownListAreaName.SelectedValue);
            conShop.ShopStartDate = Convert.ToDateTime(txtStartDate.Text);
            conShop.ShopEndDate = Convert.ToDateTime(txtEndDate.Text);
            conShop.ContactorName = txtContactName.Text.Trim();
            conShop.Tel = txtContactTel.Text.Trim();
            
            conShop.StoreID = Convert.ToInt32(this.ddlStore.SelectedValue);//商业项目
            
            if (this.txtUnits.Text.TrimEnd(',') != "")
            {
                string[] arr = this.txtUnits.Text.TrimEnd(',').Split(',');
                if (arr[0] != "")
                {
                    BaseBO objBaseBo = new BaseBO();
                    objBaseBo.WhereClause = "unitid=" + arr[0].ToString();
                    Resultset rs = objBaseBo.Query(new Units());
                    if (rs.Count == 1)
                    {
                        Units units = rs.Dequeue() as Units;
                        conShop.BuildingID = units.BuildingID;
                        conShop.FloorID = units.FloorID;
                        conShop.LocationID = units.LocationID;
                    }
                    else
                    {
                        conShop.BuildingID = 0;
                        conShop.FloorID = 0;
                        conShop.LocationID = 0;
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "ShopBaseInfo_SaveBuildingIDErr") + "'", true);
                    }
            ...
            conShop.ContractID = Convert.ToInt32(Request.Cookies["Info"].Values["conID"]);
            if (Request.QueryString["modify"] == null && (Request.Cookies["Info"]["modify"] == null || Request.Cookies["Info"]["modify"] == "")) //非修改合同
            {
            ...
            else if (Request.QueryString["modify"] == "1" || Request.Cookies["Info"]["modify"] == "1")
            {
                if (rdoYes.Checked)
                {
                    conShop.ShopStatus = ConShop.CONSHOP_TYPE_INGEAR;
                }
                else if (rdoNo.Checked)
                {
                    conShop.ShopStatus = ConShop.CONSHOP_TYPE_END;
                }
            }
    ...
    protected void GetShopUnits()
    {
        ...
        if (count > 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if ( unitDS.Tables[0].Rows.Count > 0)
                {
                    ...
                    this.txtUnits.Text = this.txtUnits.Text.Trim() + unitDS.Tables[0].Rows[0]["UnitID"].ToString() + ",";
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "message", "parent.document.all.txtWroMessage.value =  '" + (String)GetGlobalResourceObject("BaseInfo", "ShopBaseInfo_NoInConShopUnit") + "'", true);
                }
            }
                ...
                if (this.txtUnits.Text.TrimEnd(',') != "")
                {
                    ...
                    for (int i = 0; i < arr.Length; i++)
                    {
                        if (arr[i] != "")
                        {
                            ...
                            conShopUnit.UnitID = Int32.Parse(arr[i].ToString());
                            conShopUnit.RentStatus = ConShopUnit.RENTSTATUS_TYPE_YES;
                            result = baseTrans.Insert(conShopUnit);
                            //修改单元状态
                            baseTrans.WhereClause = "";
                            baseTrans.WhereClause = "UnitID = " + Convert.ToInt32(arr[i].ToString());
    ...
    private void TextEnabled(Boolean boolean)
    {
        ...
        btnSave.Enabled = boolean;
        btnBlankOut.Enabled = boolean;
        //BtUBlankOut.Enabled = boolean;
        //BtUAdd.Enabled=boolean;
        this.ddlStore.Enabled = boolean;
        this.txtUnits.Enabled = boolean;
    }
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        if (rs.Count == 1)
        {
            ...
            this.txtShopBrand.Text = objConShopBrand.BrandName;
    ...
    protected void BtnUnitsAdd_Click(object sender, EventArgs e)
    {
        ...
        {
            ListBoxUnits.Items.Clear();
            string[] arr = txtUnits.Text.TrimEnd(',').Split(',');
            for (int i = 0; i < arr.Length; i++)
            {
                if (arr[i] != "")
                {
                    BaseBO basebo = new BaseBO();
                    basebo.WhereClause = "UnitID = " + Convert.ToInt32(arr[i].ToString());
                    string sql = "select UnitID,UnitCode + ' : ' + cast(UseArea as varchar(50)) as UnitCodeArea from Unit where UnitID = " + Convert.ToInt32(arr[i].ToString());
                    DataSet UnitDS = basebo.QueryDataSet(sql);
                    DataSet dsConShop = new DataSet();
                    ...
                    if (Convert.ToInt32(ViewState["Lock"]) == SMSPara.AUTO_YES)
                    {
                        if (txtShopCode.Text.Trim() == "")
                        {
                            ...
                            if (this.txtUnits.Text.TrimEnd(',') != "")
                            {
                                string[] arrcode = this.txtUnits.Text.TrimEnd(',').Split(',');
                                if (arrcode[0] != "")
                                {
                                    BaseBO objBaseBo = new BaseBO();
                                    objBaseBo.WhereClause = "unitid=" + arrcode[0].ToString();
                                    Resultset rs = objBaseBo.Query(new Units());
                                    if (rs.Count == 1)
                                    {
                                        Units units = rs.Dequeue() as Units;
                                        shopCode = ViewState["ContractCode"].ToString() + "_" + units.BuildingID + "_" + units.UnitCode;
                                    }
                                }
                            }
                            if (shopCode.Length > 16)
