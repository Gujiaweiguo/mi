...
import { filterNavigationItems } from './auth/permissions'
...
const sessionLabel = computed(() => authStore.userDisplayName || 'Signed in')
const navigationItems = computed(() => filterNavigationItems(authStore.sessionUser))
...
const handleGlobalHttpError = (event: Event) => {
  ...
  globalError.value = detail?.message ?? 'A platform request failed.'
}

const handleAuthExpired = async () => {
  authStore.logout()
  globalError.value = 'Your session has expired. Sign in again to continue.'
  ...
  <el-container class="app-shell">
    <el-header class="app-shell__header">
      <div class="app-shell__brand">
        <span class="app-shell__eyebrow">Legacy system migration</span>
      ...
      <div class="app-shell__controls">
        <div v-if="isAuthenticated" class="app-shell__session">
          ...
          <el-button text data-testid="app-logout-button" @click="handleLogout">Logout</el-button>
        </div>

        <el-tag v-else effect="plain" type="warning">Authentication required</el-tag>
      </div>
    </el-header>
    ...
    <el-container class="app-shell__body">
      <el-aside v-if="showShellNavigation" class="app-shell__aside" width="18rem">
        <el-menu :default-active="route.path" class="app-shell__nav" router>
          ...
          <el-menu-item index="/forbidden">Access notices</el-menu-item>
        </el-menu>
      </el-aside>
