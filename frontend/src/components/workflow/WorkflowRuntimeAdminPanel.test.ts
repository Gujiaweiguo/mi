...
import WorkflowRuntimeAdminPanel from './WorkflowRuntimeAdminPanel.vue'
...
const mountView = async () => {
  const wrapper = mount(WorkflowRuntimeAdminPanel, {
...
describe('WorkflowRuntimeAdminPanel', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(listWorkflowInstances).mockResolvedValue({ data: { instances } } as never)
    ...
    vi.mocked(getWorkflowAuditHistory).mockResolvedValue({ data: { history: auditHistory } } as never)
    vi.mocked(getReminderHistory).mockResolvedValue({ data: { reminders: reminderHistory } } as never)
  ...
  it('disables runtime actions when approve permission is missing', async () => {
    canAccessMock.mockImplementation((_functionCode: string, action?: string) => action === 'view')

    const wrapper = await mountView()

    expect((wrapper.get('[data-testid="workflow-run-reminders-button"]').element as HTMLButtonElement).disabled).toBe(true)
    expect((wrapper.get('[data-testid="workflow-approve-button-101"]').element as HTMLButtonElement).disabled).toBe(true)
    expect((wrapper.get('[data-testid="workflow-reject-button-101"]').element as HTMLButtonElement).disabled).toBe(true)
    expect((wrapper.get('[data-testid="workflow-resubmit-button-102"]').element as HTMLButtonElement).disabled).toBe(true)
  })
})
