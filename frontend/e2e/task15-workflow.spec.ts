...
test('completes the task 15 lease to billing output workflow with stable test ids', async ({ page }) => {
  ...
  await leaseForm.getByRole('textbox', { name: /合同编号/ }).fill('LS-2026-001')
  await leaseForm.getByRole('textbox', { name: /租户名称/ }).fill('Harbor Foods')
  ...
  await leaseForm.getByRole('spinbutton', { name: /业态 ID/ }).fill('102')
  await leaseForm.getByRole('spinbutton', { name: /经营方式 ID/ }).fill('101')
  await leaseForm.getByRole('combobox', { name: /开始日期/ }).fill('2026-01-01')
  await leaseForm.getByRole('combobox', { name: /结束日期/ }).fill('2026-12-31')
  await leaseForm.getByRole('spinbutton', { name: /单元 ID/ }).fill('301')
  await leaseForm.getByRole('spinbutton', { name: /租赁面积/ }).fill('120')
  await leaseForm.getByRole('spinbutton', { name: /币种 ID/ }).fill('1')
  await leaseForm.getByRole('spinbutton', { name: /^\*?金额$/ }).fill('8000')
  await leaseForm.getByRole('combobox', { name: /生效开始/ }).fill('2026-01-01')
  ...
  const taxExportDateInputs = page.locator('.tax-exports-view__export-grid .el-date-editor input')
  const taxExportFromInput = taxExportDateInputs.nth(0)
  const taxExportToInput = taxExportDateInputs.nth(1)
  await taxExportFromInput.click()
  await taxExportFromInput.fill('2026-01-01')
  await taxExportFromInput.press('Tab')
  await taxExportToInput.click()
  await taxExportToInput.fill('2026-01-31')
  await taxExportToInput.press('Tab')
  await page.getByTestId('tax-export-button').click()
  await expect(page.getByTestId('tax-exports-feedback')).toContainText('TAX-LEASE')
